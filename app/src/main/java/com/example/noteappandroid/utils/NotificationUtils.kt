...
fun NotificationManager.sendReminderNotification(
    ...
    val style = NotificationCompat.BigTextStyle().bigText(note.noteText?.ifBlank { note.title })
...
//        .setSubText(folder.getTitle(context))
        .setStyle(style).setColorized(true).setCategory(Notification.CATEGORY_REMINDER)
        .setSmallIcon(R.drawable.ic_round_notifications_active_24)
        .setPriority(NotificationCompat.PRIORITY_HIGH).setAutoCancel(true).setGroupSummary(true)
        .build()
    Log.d(
        "TEST",
        "dfdfdf ${note.id}"
...
fun NotificationManager.createNotificationChannelsUtil(context: Context) {
    NotificationChannel(
        RemindersChannelId,
